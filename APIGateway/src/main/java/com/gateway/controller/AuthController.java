...
import com.gateway.payload.AuthResponse;
...
public class AuthController {
	...
	public ResponseEntity<AuthResponse> login(@RegisteredOAuth2AuthorizedClient("okta") OAuth2AuthorizedClient client,@AuthenticationPrincipal OidcUser user,
		...
		List<String> authorties =user.getAuthorities().stream().map(t ->t.getAuthority() ).collect(Collectors.toList());
		
		authResponse.setAuthorities(authorties);
